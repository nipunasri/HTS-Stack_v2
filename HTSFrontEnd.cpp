...
UINT CFrontEnd::uiBuildOCLKernels()
{
  ...
  size_t           uiReqQueueSize  = OCL_REQ_QUEUE_SIZE*sizeof(TQueuedRequest); // 4096
  size_t           uiNodePoolSize  = OCL_NODE_POOL_SIZE*sizeof(TLLNode); // 1048576
  size_t           uiHashTableSize = OCL_HASH_TABLE_SIZE*sizeof(TLLNode); // 1024
  ...
  for(UINT i = 0; i < OCL_HASH_TABLE_SIZE; ++i)
    {
      pNodePool[i].uiNext = 0; // TODO : do i need to put freebit 1 ??
    }

  pMiscData->uiReadIndex  = OCL_HASH_TABLE_SIZE;
  pMiscData->uiWriteIndex = OCL_HASH_TABLE_SIZE + 1;  
  pMiscData->uiHeadIndex = OCL_HASH_TABLE_SIZE ; // For HTSKernels_v1.cl - 17Nov15

  cl_uint uiStartIndex = OCL_HASH_TABLE_SIZE + 2;
  cl_uint uiEndIndex   = OCL_HASH_TABLE_SIZE + OCL_NODE_POOL_SIZE -1;  

  //for(cl_uint i = uiStartIndex; i < uiEndIndex; ++i) <-- original
  for(cl_uint i = OCL_HASH_TABLE_SIZE; i < uiEndIndex; ++i) // testing SNIP : remove +5 later
    {
      cl_uint j = SET_MPTR((i+1),0x4); // set freeBit:100
      pNodePool[i].uiNext = j;
    }  
  pNodePool[uiEndIndex].uiNext              = 0;
  // Can statically assign values to nodes in nodepool for testing purpose - TESTING

  pNodePool[0].uiNext = SET_PTR(1025) ;
  pNodePool[1025].uiNext = 0 ;

  for(int i = 0 ; i <= 255; i++) {
	  pNodePool[0].pE[i] = 1024*(i+1) ;
  }
  for(int i = 0; i <= 255; i++) 
	  pNodePool[1025].pE[i] = 1024*(257+i) ;

  pNodePool[1024].uiNext = 1026 ; ////must require
  ...
  if(uiFlags & HTS_REQ_COMPLETED)
    {
      (tFid->pThreadRequest[uiReqId]).uiFlags = 0;
      *pStatus   = uiRStatus;
	   // can print Node's data here : PRINT
	  //for(int i = 0; i <= 255; i++)
		  //std::cout<<(pNodePool[1026].pE[i])<<" " ;
	  //std::cout << std::endl ;
	  //std::cout<<(pNodePool[1024].uiNext>>3)<<std::endl ;
